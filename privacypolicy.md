# Privacy Policy / Gizlilik Politikası

**Last Updated / Son Güncelleme: December 5, 2025**

---

## 🇹🇷 TÜRKÇE

### 1. Giriş

One Minute Digital Diary ("Uygulama", "biz", "bizim") olarak gizliliğinize saygı duyuyor ve kişisel verilerinizi korumayı taahhüt ediyoruz. Bu Gizlilik Politikası, uygulamamızı kullandığınızda hangi bilgileri topladığımızı, nasıl kullandığımızı ve koruduğumuzu açıklamaktadır.

Bu politika, 6698 sayılı Kişisel Verilerin Korunması Kanunu (KVKK), Avrupa Birliği Genel Veri Koruma Tüzüğü (GDPR) ve California Tüketici Gizlilik Yasası (CCPA) dahil olmak üzere uluslararası veri koruma düzenlemelerine uygun olarak hazırlanmıştır.

### 2. Topladığımız Veriler

#### 2.1 Doğrudan Sağladığınız Bilgiler
- **Hesap Bilgileri**: E-posta adresi, kullanıcı adı
- **Profil Bilgileri**: Profil fotoğrafı (isteğe bağlı)
- **İçerik**: Video günlükleriniz, başlıklar ve açıklamalar

#### 2.2 Otomatik Olarak Toplanan Bilgiler
- **Cihaz Bilgileri**: Cihaz türü, işletim sistemi versiyonu
- **Kullanım Verileri**: Uygulama kullanım istatistikleri, oturum süreleri
- **Teknik Veriler**: IP adresi, tarayıcı türü, dil tercihleri

#### 2.3 Üçüncü Taraf Hizmetleri
Aşağıdaki üçüncü taraf hizmetlerini kullanıyoruz:
- **Firebase Authentication**: Kimlik doğrulama
- **Firebase Firestore**: Veri depolama
- **Firebase Storage**: Video ve medya depolama
- **Firebase Cloud Messaging**: Bildirimler
- **Google Play Billing**: Uygulama içi satın alma yönetimi

### 3. Verilerin Kullanım Amaçları

Verilerinizi aşağıdaki amaçlarla kullanıyoruz:
- Hesabınızı oluşturmak ve yönetmek
- Video günlüklerinizi güvenli şekilde saklamak
- Uygulama deneyiminizi kişiselleştirmek
- Teknik destek sağlamak
- Uygulama güvenliğini ve performansını iyileştirmek
- Yasal yükümlülüklerimizi yerine getirmek
- Premium abonelik hizmetlerini yönetmek

### 4. Verilerin Saklanması ve Güvenliği

#### 4.1 Saklama Süresi
- Hesap bilgileriniz, hesabınızı silene kadar saklanır
- Video içerikleriniz, siz silene kadar veya hesap silme işlemine kadar saklanır
- Teknik loglar 90 gün süreyle saklanır

#### 4.2 Güvenlik Önlemleri
- Tüm veriler şifreli bağlantılar (SSL/TLS) üzerinden iletilir
- Videolar ve kişisel veriler şifreli olarak saklanır
- Firebase güvenlik kuralları ile yetkisiz erişim engellenir
- Düzenli güvenlik denetimleri yapılır

### 5. Veri Paylaşımı

Kişisel verilerinizi aşağıdaki durumlar dışında üçüncü taraflarla paylaşmıyoruz:
- **Hizmet Sağlayıcılar**: Firebase (Google) teknik altyapı sağlayıcıları
- **Yasal Zorunluluklar**: Mahkeme kararı veya yasal düzenleme gerektirdiğinde
- **İzniniz ile**: Açık onayınız olduğunda

### 6. Kullanıcı Hakları

#### 6.1 KVKK Kapsamındaki Haklarınız (Türkiye)
- Kişisel verilerinizin işlenip işlenmediğini öğrenme
- İşlenmişse buna ilişkin bilgi talep etme
- İşlenme amacını ve bunların amacına uygun kullanılıp kullanılmadığını öğrenme
- Eksik veya yanlış işlenmişse düzeltilmesini isteme
- Kişisel verilerinizin silinmesini veya yok edilmesini isteme
- İşlenen verilerin münhasıran otomatik sistemler vasıtasıyla analiz edilmesi suretiyle aleyhinize bir sonucun ortaya çıkmasına itiraz etme

#### 6.2 GDPR Kapsamındaki Haklarınız (Avrupa)
- **Erişim Hakkı**: Verilerinize erişim talep etme
- **Düzeltme Hakkı**: Yanlış verilerin düzeltilmesini isteme
- **Silme Hakkı (Unutulma Hakkı)**: Verilerinizin silinmesini talep etme
- **İşlemeyi Kısıtlama Hakkı**: Veri işlemenin kısıtlanmasını isteme
- **Veri Taşınabilirliği Hakkı**: Verilerinizi yapılandırılmış formatta alma
- **İtiraz Hakkı**: Belirli işleme faaliyetlerine itiraz etme

#### 6.3 CCPA Kapsamındaki Haklarınız (California, ABD)
- Toplanan kişisel bilgileri öğrenme hakkı
- Kişisel bilgilerin silinmesini talep etme hakkı
- Kişisel bilgilerin satışından vazgeçme hakkı (Not: Kişisel bilgilerinizi satmıyoruz)
- Haklarınızı kullandığınız için ayrımcılığa uğramama hakkı

### 7. Çocukların Gizliliği

Uygulamamız 13 yaşın altındaki çocuklara yönelik değildir. Bilerek 13 yaşın altındaki çocuklardan kişisel veri toplamıyoruz. Eğer 13 yaşın altında bir çocuğun bize kişisel bilgi sağladığını fark ederseniz, lütfen bizimle iletişime geçin.

### 8. Uluslararası Veri Transferleri

Verileriniz, Firebase altyapısı aracılığıyla Amerika Birleşik Devletleri ve diğer ülkelerdeki sunucularda işlenebilir. Bu transferler, uygun güvenlik önlemleri ve yasal mekanizmalar (Standart Sözleşme Maddeleri gibi) ile korunmaktadır.

### 9. Çerezler ve İzleme Teknolojileri

Uygulamamız doğrudan çerez kullanmamaktadır. Ancak, Firebase ve diğer üçüncü taraf hizmetleri, analitik ve performans izleme amacıyla cihaz tanımlayıcıları kullanabilir.

### 10. Politika Değişiklikleri

Bu Gizlilik Politikası'nı zaman zaman güncelleyebiliriz. Önemli değişiklikler olması durumunda, uygulama içi bildirim veya e-posta yoluyla sizi bilgilendireceğiz. Güncellenmiş politika, yayınlandığı tarihte yürürlüğe girer.

### 11. İletişim

Gizlilik ile ilgili sorularınız için:
- **E-posta**: [rulingg2@gmail.com]

---

## 🇬🇧 ENGLISH

### 1. Introduction

One Minute Digital Diary ("App", "we", "our") respects your privacy and is committed to protecting your personal data. This Privacy Policy explains what information we collect when you use our app, how we use it, and how we protect it.

This policy has been prepared in compliance with international data protection regulations including the European Union General Data Protection Regulation (GDPR), California Consumer Privacy Act (CCPA), and Turkish Personal Data Protection Law (KVKK).

### 2. Data We Collect

#### 2.1 Information You Provide Directly
- **Account Information**: Email address, username
- **Profile Information**: Profile picture (optional)
- **Content**: Your video diaries, titles, and descriptions

#### 2.2 Information Collected Automatically
- **Device Information**: Device type, operating system version
- **Usage Data**: App usage statistics, session durations
- **Technical Data**: IP address, browser type, language preferences

#### 2.3 Third-Party Services
We use the following third-party services:
- **Firebase Authentication**: Identity verification
- **Firebase Firestore**: Data storage
- **Firebase Storage**: Video and media storage
- **Firebase Cloud Messaging**: Notifications

### 3. How We Use Your Data

We use your data for the following purposes:
- To create and manage your account
- To securely store your video diaries
- To personalize your app experience
- To provide technical support
- To improve app security and performance
- To fulfill our legal obligations
- To manage premium subscription services

### 4. Data Storage and Security

#### 4.1 Retention Period
- Your account information is stored until you delete your account
- Your video content is stored until you delete it or until account deletion
- Technical logs are retained for 90 days

#### 4.2 Security Measures
- All data is transmitted over encrypted connections (SSL/TLS)
- Videos and personal data are stored encrypted
- Unauthorized access is prevented through Firebase security rules
- Regular security audits are conducted

### 5. Data Sharing

We do not share your personal data with third parties except in the following circumstances:
- **Service Providers**: Technical infrastructure providers such as Firebase (Google)
- **Legal Requirements**: When required by court order or legal regulation
- **With Your Consent**: When you give explicit consent

### 6. User Rights

#### 6.1 Your Rights Under GDPR (Europe)
- **Right of Access**: Request access to your data
- **Right to Rectification**: Request correction of inaccurate data
- **Right to Erasure (Right to be Forgotten)**: Request deletion of your data
- **Right to Restrict Processing**: Request restriction of data processing
- **Right to Data Portability**: Receive your data in a structured format
- **Right to Object**: Object to certain processing activities

#### 6.2 Your Rights Under CCPA (California, USA)
- Right to know what personal information is collected
- Right to request deletion of personal information
- Right to opt-out of the sale of personal information (Note: We do not sell your personal information)
- Right to non-discrimination for exercising your rights

#### 6.3 Your Rights Under KVKK (Turkey)
- Right to learn whether your personal data is processed
- Right to request information if processed
- Right to learn the purpose of processing and whether they are used appropriately
- Right to request correction if incomplete or incorrect
- Right to request deletion or destruction of your personal data
- Right to object to a result arising against you through analysis of processed data exclusively by automated systems

### 7. Children's Privacy

Our app is not intended for children under the age of 13. We do not knowingly collect personal data from children under 13. If you become aware that a child under 13 has provided us with personal information, please contact us.

### 8. International Data Transfers

Your data may be processed on servers in the United States and other countries through the Firebase infrastructure. These transfers are protected by appropriate security measures and legal mechanisms (such as Standard Contractual Clauses).

### 9. Cookies and Tracking Technologies

Our app does not directly use cookies. However, Firebase and other third-party services may use device identifiers for analytics and performance monitoring purposes.

### 10. Policy Changes

We may update this Privacy Policy from time to time. In case of significant changes, we will notify you through in-app notification or email. The updated policy becomes effective on the date it is published.

### 11. Contact

For privacy-related questions:
- **Email**: [rulingg2@gmail.com]

---

## 🇩🇪 DEUTSCH

### 1. Einleitung

One Minute Digital Diary ("App", "wir", "unser") respektiert Ihre Privatsphäre und verpflichtet sich zum Schutz Ihrer personenbezogenen Daten. Diese Datenschutzrichtlinie erläutert, welche Informationen wir bei der Nutzung unserer App erfassen, wie wir sie verwenden und wie wir sie schützen.

Diese Richtlinie wurde in Übereinstimmung mit internationalen Datenschutzbestimmungen erstellt, einschließlich der Datenschutz-Grundverordnung (DSGVO) der Europäischen Union.

### 2. Daten, die wir erfassen

#### 2.1 Von Ihnen direkt bereitgestellte Informationen
- **Kontoinformationen**: E-Mail-Adresse, Benutzername
- **Profilinformationen**: Profilbild (optional)
- **Inhalte**: Ihre Video-Tagebücher, Titel und Beschreibungen

#### 2.2 Automatisch erfasste Informationen
- **Geräteinformationen**: Gerätetyp, Betriebssystemversion
- **Nutzungsdaten**: App-Nutzungsstatistiken, Sitzungsdauern
- **Technische Daten**: IP-Adresse, Browsertyp, Spracheinstellungen

### 3. Ihre Rechte nach DSGVO

- **Auskunftsrecht**: Zugang zu Ihren Daten anfordern
- **Recht auf Berichtigung**: Berichtigung unrichtiger Daten verlangen
- **Recht auf Löschung**: Löschung Ihrer Daten beantragen
- **Recht auf Einschränkung der Verarbeitung**: Einschränkung der Datenverarbeitung verlangen
- **Recht auf Datenübertragbarkeit**: Ihre Daten in strukturiertem Format erhalten
- **Widerspruchsrecht**: Bestimmten Verarbeitungstätigkeiten widersprechen

### 4. Kontakt

Für datenschutzbezogene Fragen:
- **E-Mail**: [rulingg2@gmail.com]

**Effective Date / Yürürlük Tarihi**: December 5, 2025
**Version / Versiyon**: 1.0

© 2025 One Minute Digital Diary. All rights reserved.
